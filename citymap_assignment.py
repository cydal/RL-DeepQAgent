...
class CarBrain:
    ...
    def get_state(self):
        sensor_vals = []
        self.sensor_coords = []
        angles = [i * SENSOR_ANGLE for i in range(-3, 4)]
        
        for a in angles:
            rad = math.radians(self.car_angle + a)
            hit_x = self.car_pos.x() + math.cos(rad) * SENSOR_DIST
            hit_y = self.car_pos.y() + math.sin(rad) * SENSOR_DIST
            sensor_reading = 1.0

            step_size = 5
            n_steps = int(SENSOR_DIST / step_size)
            for i in range(1, n_steps + 1):
                dist_now = i * step_size
                px = self.car_pos.x() + math.cos(rad) * dist_now
                py = self.car_pos.y() + math.sin(rad) * dist_now
            ...
            self.sensor_coords.append(QPointF(hit_x, hit_y))
            sensor_vals.append(sensor_reading)
...
class CarItem(QGraphicsItem):
    def __init__(self):
        ...
        for candidate in ("car.png", "car.png.jpg", "car.jpg", "car.jpeg"):
    ...
    def paint(self, painter, option, widget):
        if self.car_pixmap is not None:
            pm = self.car_pixmap.scaled(int(self.car_w), int(self.car_h), Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
